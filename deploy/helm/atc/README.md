...
> **No built-in authentication.** ATC does not gate the SPA, `/v1/state`, or `/v1/ws`. Deploy the chart behind a trusted network surface (private VPC, VPN, Tailscale) or an authenticating reverse proxy. See [`docs/operator/authentication.md`](../../../docs/operator/authentication.md) for per-proxy recipes (Pomerium recommended; oauth2-proxy and Authelia work; Cloudflare Access is unsupported for the live WebSocket route) and the webhook-endpoint split.
