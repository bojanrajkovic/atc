...
- **Runner-pool capacities gating:** `runnerPools` (default `[]`) drives `templates/configmap.yaml` and the `runner-pool-config` volume mount in `templates/deployment.yaml`. Empty list ⇒ no `ConfigMap`, no volume, byte-identical to prior chart behavior. Non-empty ⇒ a `<release>-atc-config` `ConfigMap` is rendered with `data."config.yaml"` containing `runner_pools:` followed by the operator-supplied entries; the deployment mounts the ConfigMap read-only at `/etc/atc/config.yaml` via `subPath: config.yaml`. `values.schema.json` is the canonical contract surface for the per-entry shape (`labels: array minItems=1 uniqueItems=true`, `capacity: integer minimum=1`, both required); server-side `Config::load` additionally canonicalizes (sort + dedup) and rejects duplicate canonicalized label sets. Canonical write-up: `docs/architecture/deployment.md` § "File-based configuration".
