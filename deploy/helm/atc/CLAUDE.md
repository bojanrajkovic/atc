...
- **HPA gating:** `autoscaling.enabled` renders `templates/hpa.yaml` (`autoscaling/v2`) and drops `spec.replicas` from the Deployment so the HPA owns the count. The multi-replica fail-guard extends to `autoscaling.maxReplicas > 1`. Canonical write-up: `docs/architecture/deployment.md` § Autoscaling.
