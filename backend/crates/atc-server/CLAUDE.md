...
| Module | Role |
|--------|------|
| `main` | Entry point, config loading, AppState creation (5 fields: `persist`, `webhook_secret`, `runner_pool_capacities`, `shutdown`, `ws_tracker`), router setup, lifecycle. The `runner_pool_capacities` field is built once at startup by mapping the validated `Config::runner_pools` into `Vec<RunnerPoolCapacity>`. Constructs the active `Arc<dyn PersistentStore>` via `PgStore::start` (PG mode) or `InMemoryStore::start` (in-memory mode); the per-store background tasks are spawned and owned by the store itself (ADR 0006). |
| `config` | figment-based `Config` chained `defaults → Yaml::file($ATC_CONFIG_FILE | "/etc/atc/config.yaml") → Env::prefixed("ATC_").split("__")`. Carries `GitHubConfig` and operator-declared `runner_pools: Vec<RunnerPoolConfig>` (file-only). `database_listener_url` falls back to `database_url` at runtime. Validation canonicalizes runner-pool labels (sort + dedup) and rejects `capacity == 0`, empty labels, and duplicate canonicalized label sets at startup. |
| `db` | `init_pool(url)` — connects sqlx PgPool and runs embedded migrations |
| `routes` | HTTP route handlers: webhook ingestion, REST state snapshot, WebSocket upgrade, health/ready probes. `state_handler` reads from `PersistentStore::read_snapshot`, then composes `runner_pool_capacities` from `AppState` onto the response so the wire payload includes operator config alongside event-derived state. |
| `state` | `AppState` struct and broadcast types (`SeqEvent`, `StateSnapshot`). `StateSnapshot` carries a `runner_pool_capacities: Vec<RunnerPoolCapacity>` field annotated `#[serde(default)]` for rolling-deploy tolerance; the field is composed onto each response in `routes::state_handler` from `AppState::runner_pool_capacities`, **not** by the store. WS subscribe seam moved to `persist.subscribe()` per ADR 0006. |
...
| `metrics` | OTel metric registration: `register_build_info` (one-shot startup gauge) and `PgMetrics::register` (cached OTel `Counter`/`Gauge`/`Histogram` instruments plus pre-built `[KeyValue; N]` attribute slices for every `atc_pg_*` emit site, owned by `PgStore`); plus `spawn_process_collector`, which wraps `opentelemetry-system-metrics::init_process_observer` in a `ProcessCollectorHandle` (abort + join). All emit sites resolve through the OTel global meter installed by `otel::init_otel`. See `docs/architecture/metrics.md` for the canonical surface, the authoring contract, and the cached-instrument convention. |
