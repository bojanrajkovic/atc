...
Last verified: 2026-05-06 (Phase 3c: PG-mode webhook handler is write-only — no in-memory apply, no broadcast, no seq mutex; drain is sole broadcaster; REPEATABLE READ snapshot for `/v1/state`; gap-healing backstop `min_pending_seq`; drain heartbeat `last_drain_pass_at`; ring-buffer dedup)
...
- **State snapshot (PG mode):** Opens a REPEATABLE READ transaction, reads runs (`WHERE placeholder=false`), jobs, and `MAX(outbox.seq)` atomically. `lastSeq` and `entity_count` come from the same MVCC snapshot, so they must agree exactly: `entity_count == lastSeq`. `lastSeq=0` means no committed outbox rows. Placeholder stub runs (created by job-before-run FK stubs) are always excluded.
...
- **Phase 3c tests** (`phase_3c_state_pg_read`, `phase_3c_webhook_pg_write_only`, `phase_3c_drain_broadcast`, `phase_3c_heartbeat`, `phase_3c_gap_healing`) — Dedicated PG-backed tests for Phase 3c contracts: REPEATABLE READ state snapshot (T1/T1b/T2/T3), PG write-only handler (T4/T4b/T5), drain broadcast (T8/T9/T10/T11), drain heartbeat (T8b/T10b), and gap-healing backstop + dedup ring buffer (T6/T6b/T7). T2 uses `tokio::join!` to fire webhook and GET /v1/state concurrently and asserts `entity_count == lastSeq` (strong equality). T6b is marked `#[ignore = "heavy"]`.
