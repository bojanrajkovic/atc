...
pub fn spawn_listener_task(
    ...
    // No task-lifetime wrapper here: `handle_listener_notification` is
    // `#[tracing::instrument(name = "listener.recv", …)]`, so each NOTIFY emits
    // its own root span on return. A long-lived `.instrument(span)` here would
    // never close (the SDK only exports on close), so the span would be lost
    // on SIGKILL/OOM. See `docs/architecture/metrics.md` § "Task-lifetime root
    // spans are an anti-pattern".
...
pub fn spawn_drain_task(
    ...
    // No task-lifetime wrapper here: `drain_pass` is
    // `#[tracing::instrument(name = "drain.pass", …)]`, so each pass emits its
    // own root span on return; `drain.broadcast` stays nested under
    // `drain.pass` via the `broadcast_span.in_scope(...)` block below. See
    // `docs/architecture/metrics.md` § "Task-lifetime root spans are an
    // anti-pattern" for why a long-lived `.instrument(span)` here would never
    // export.
    tokio::spawn(async move {
