...
**Jobs are filtered to the run's current attempt on read, not deleted on re-run.** GitHub assigns fresh job IDs per attempt under the reused `run_id`, so a re-run's job rows accumulate alongside the prior attempt's. `jobs.run_attempt` (migration `0009`) records each job's attempt; `read_all_jobs` joins `runs` and filters `WHERE j.run_attempt = r.run_attempt` so prior-attempt jobs drop out of the snapshot. Filtering (not deleting) is deliberate — it is safe under webhook reordering, where an attempt-2 job event can arrive before the attempt-2 run event. `atc-store-mem` applies the same parent-attempt filter in `read_snapshot_inner`.
