...
**Jobs are filtered to the parent run's current attempt on read.** `read_snapshot_inner` drops jobs whose `run_attempt` differs from their parent run's (when the parent is known) so a re-run's card doesn't mix the prior attempt's jobs with the new ones. Jobs with an absent parent (job-before-run stub) are kept — the attempt can't be compared yet and self-heals once the run event lands. Mirrors `atc-store-pg`'s `j.run_attempt = r.run_attempt` read filter.
