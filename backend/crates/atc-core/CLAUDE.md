...
**`run_attempt` is carried, not interpreted — the FSM stays forward-only.** `RunEventEnvelope` and `WorkflowRun` both carry `run_attempt: i32` (1-based; GitHub increments it on re-runs while reusing the same `run_id`). `apply_run_event` only copies the envelope's `run_attempt` onto the resulting run; it does **not** treat a higher attempt as a reason to reset terminal state. Re-run detection is deliberately a persistence concern (the store decides whether to pass `None` for a fresh start or to reset columns) — keeping it out of atc-core preserves the forward-only, side-effect-free transition contract. Do not add attempt-comparison branching to the pure functions.
