...
**`src/lib/test-utils/factories.ts` must stay value-import-free.** E2E consumers import the factories by relative path because Playwright resolves relative imports but not the `$lib` tsconfig alias for *value* imports — e2e specs use `../src/lib/test-utils/factories`, and `e2e/lib/ws-mock.ts` (which builds wire JSON via `createMockRunEvent`) uses `../../src/lib/test-utils/factories` from one level deeper. The factory module works there only because all its imports are `import type` (erased at transpile, so nothing hits the resolver). Add a single value import — e.g. a shared default constant or a runtime enum — and every e2e file throws a module-resolution error at Playwright runtime with **no compile-time signal** (vitest resolves `$lib` through Vite and svelte-check through tsconfig `paths`, so both stay green; the break only surfaces in the separate Playwright CI job). Keep shared defaults inside the factory file. See `docs/architecture/frontend-app.md` § Test Strategy.
