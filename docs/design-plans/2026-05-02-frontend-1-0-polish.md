...
- **frontend-1-0-polish.AC7.1 Success:** A vitest browser-mode test uses a manually-driven RAF queue (via `vi.stubGlobal('requestAnimationFrame', ...)`) — NOT `vi.useFakeTimers()`: enqueues 1000 mock events in 10 batches of 100, advances the RAF queue exactly once per batch via a `tickRAF()` helper, asserts `flushSpy.mock.calls.length === N` (deterministic, not bounded), asserts `runStore.runs.size === 1000` (every event landed in store state), and asserts no events are dropped (`totalEventsReceived === 1000`). Failure is a CI hard fail. Implementation note: the manually-driven RAF queue (`rafQueue.splice(0)` + invoke) provides full determinism without needing fake timers — each `tickRAF()` call drains exactly one batch, giving precise control of when batches close and eliminating all wall-clock dependency.
