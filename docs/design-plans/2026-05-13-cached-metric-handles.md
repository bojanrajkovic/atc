...
7. **Final verification.** `just lint` (clippy + biome + rustfmt), `just test` (backend nextest + frontend Vitest), and the emit-site grep gate from AC1 (`rg -U --multiline 'metrics::(counter|gauge|histogram)!\([\s\S]*?\)\s*\.(increment|set|record)' backend/crates/atc-server/src/`) returns only the `register_build_info` emit in `metrics.rs`.

## Acceptance Criteria

- **AC1.** The emit-site grep `rg -U --multiline 'metrics::(counter|gauge|histogram)!\([\s\S]*?\)\s*\.(increment|set|record)' backend/crates/atc-server/src/` returns **only** the `register_build_info` gauge in `metrics.rs`. The bare `metrics::(counter|gauge|histogram)!(…)` macro form without a chained `.increment` / `.set` / `.record` legitimately appears inside `PgMetrics::register` — there the macros are the API for obtaining a cached handle, not an emit. The chained form is the emit pattern this AC forbids outside the one exception, and every other production emit goes through a `PgMetrics` field access.
