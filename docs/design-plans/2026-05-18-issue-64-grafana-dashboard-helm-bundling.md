...
- **Histogram query form:** keep `histogram_quantile(0.99, sum(rate(name_bucket[5m])) by (le, pod))` — the classic-histogram form. The OTel SDK uses `Aggregation::Base2ExponentialHistogram` (see `backend/crates/atc-server/src/otel.rs:169` and `:199`), but the OTLP→Prometheus path in the operator-run collector emits classic histograms by default. The existing dashboard's `_bucket` queries work today, which is the empirical confirmation. `docs/architecture/metrics.md` § Histogram aggregation currently overstates that native-histogram emission "continues to work" — the claim is misleading because native histograms have no `_bucket` series. Correct that doc as part of this PR. Operators running the collector with native-histogram-only emission will need to translate panel queries to `histogram_quantile(0.99, sum(rate(name[5m])))` (no `_bucket`, no `le` group); document this caveat in the dashboard's `description` field.
