...
2. The cleanup edits, all on the same branch:
   ...
   - `.github/workflows/ci.yml` and `.github/workflows/release-please.yml`: move `# zizmor: ignore[superfluous-actions]` to its own line above the `uses:` line for `dtolnay/rust-toolchain`
...
| `.github/workflows/ci.yml` line 77 | Move `# zizmor: ignore[superfluous-actions]` to a comment line directly above the `uses: dtolnay/rust-toolchain@...`. |
| `.github/workflows/release-please.yml` line 86 | Same restructure. |
| `.github/workflows/ci.yml` helm-install job (around line 353) | Add a "Read Rust + Node versions from mise.toml" step before the `docker build` step — extract `tools.rust` and `tools.node` via `mise config get` (mirroring the existing setup-node / rust-toolchain extraction at ci.yml:71–75 and 169–171). Update `docker build -t atc:ct -f Dockerfile .` to `docker build -t atc:ct -f Dockerfile --build-arg RUST_VERSION=<rust> --build-arg NODE_VERSION=<node> .` so the built image's actual versions track `.mise.toml` (the documented intent — see Dockerfile header comment). The Dockerfile's ARG defaults remain as fallbacks for standalone `docker build`. |
...
3. **Apply cleanup edits in dependency order:**
   ...
   - Edit `.github/workflows/ci.yml` line 77 (zizmor comment move).
   - Edit `.github/workflows/release-please.yml` line 86 (zizmor comment move).
   - Edit `.github/workflows/ci.yml` helm-install job: insert a step that reads `tools.rust` and `tools.node` from `.mise.toml` via `mise config get`, then thread them through `--build-arg` on the `docker build` command. Pattern mirrors the existing rust-toolchain / setup-node extraction at ci.yml:71–75 and 169–171.
...
AC6. `.github/workflows/ci.yml:77` and `.github/workflows/release-please.yml:86` (or wherever they land after the edit) — `# zizmor: ignore[superfluous-actions]` is on its own line above the `uses:` line. The action SHA + `# vX.Y.Z` comment is preserved on the `uses:` line.

AC6b. `.github/workflows/ci.yml` helm-install job extracts `tools.rust` and `tools.node` from `.mise.toml` via `mise config get` and passes them via `--build-arg RUST_VERSION=… --build-arg NODE_VERSION=…` to the `docker build -t atc:ct -f Dockerfile .` invocation. The extraction step uses `>> "$GITHUB_OUTPUT"` and is referenced via `${{ steps.<id>.outputs.<name> }}` in the build step, mirroring ci.yml:71–75. After this change, a `mise.toml` rust/node bump that lands without a corresponding Dockerfile ARG default bump still produces a CI build using the mise-pinned version (the ARG default is fallback-only).
...
| `docs/architecture/release-pipeline.md` | Required by `scripts/doc-mapping.sh:69` because this PR edits `.github/workflows/release-please.yml` (the zizmor comment move). Update: append a short note that the workflow's `dtolnay/rust-toolchain` step carries a zizmor ignore on its own line; no behavior change. |
