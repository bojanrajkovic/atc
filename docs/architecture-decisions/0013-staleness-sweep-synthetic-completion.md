...
- **Pass 2 — runs.** Candidates: `status != 'Completed' AND placeholder = false AND updated_at < cutoff AND NOT EXISTS (non-terminal jobs at run_attempt or higher for this run)`. `placeholder = false` excludes FK-stub rows (`0003_runs_placeholder.sql`) — `read_all_runs` already hides them from `/v1/state` regardless of status, so sweeping one would only spend work turning an invisible stub into an invisible `Stale` stub, and `upsert_run_in_txn` unconditionally flips `placeholder` to `false` on any write, which would be actively wrong to do to a stub. The `NOT EXISTS` guard matters: `runs.updated_at` only bumps on **run** webhooks, so a legitimately-running multi-day self-hosted job would otherwise starve its run's signal and get its parent falsely swept. Because jobs sweep first in the same tick, any *stale* jobs are already force-completed by the time pass 2 runs its `NOT EXISTS` query — any remaining non-terminal job is fresh and correctly shields its run. The `jobs.run_attempt >= runs.run_attempt` filter (mirroring `read_all_jobs`'s read-path filter) matters for re-runs specifically: a `Waiting` job can never be swept (excluded from pass 1's candidacy — see below), so a `Waiting` job abandoned on a superseded lower attempt would otherwise shield the *current* attempt's run from ever being swept, forever.

Per candidate row, one transaction: `SELECT ... FOR UPDATE SKIP LOCKED` → re-check the row is *still non-terminal* AND *still stale by the age predicate* (and, for runs, that it still has no live jobs) → if so, build a synthetic `RunEventEnvelope`/`JobEventEnvelope` (`Completed { conclusion: Stale }`, `completed_at = now`) sourced entirely from the locked row → write through the same `upsert_*_in_txn` + `insert_outbox_*_in_txn` + `notify_outbox_seq_in_txn` helpers the webhook handler uses → commit. The age recheck matters on its own: a real *progress* webhook (e.g. `Queued -> InProgress`, bumping `started_at`/`updated_at`) doesn't complete the row, so the status check alone would still let a just-started job or run through — "not yet `Completed`" isn't the same claim as "still past the threshold" once the candidate-select snapshot is stale.

- **Race vs. a real webhook:** the `FOR UPDATE` + status/age re-check closes it. If the real `Completed{Success}` commits first, the sweep observes `Completed` and skips. If a real progress event (not a completion) commits first, the sweep observes fresh activity and skips. If the sweep commits first, the real webhook later lands as an idempotent `Completed → Completed` and its conclusion wins.
...
The same policy is expressed as a pure predicate pair in `atc-core` beside `is_evictable` — `is_stale_job` / `is_stale_run` — and `atc-store-mem` wires them into its existing eviction-tick task (same interval, same tick, no separate task).

There is no row lock concept in the in-memory store, so the race against a concurrent real event is closed differently, but with the same guarantee PG's row lock gives: `apply_run_event`/`apply_job_event`'s existing mutation logic is factored into private `_locked` helpers that take an already-acquired `state` write-lock guard rather than acquiring their own. The public `PersistentStore::apply_*_event` methods acquire the lock and call the helper as before; the sweep's `sweep_one_job_if_stale`/`sweep_one_run_if_stale` acquire the *same* lock themselves, re-check staleness against the state they're already holding, and — only if still stale — call the identical helper, all within one uninterrupted critical section. This matters because `Completed -> Completed` is an *admitted* idempotent replay by design (real webhook redelivery depends on it), not a rejected transition: an earlier version of this sweep re-checked staleness and then made a *separate* call into the public `apply_job_event`/`apply_run_event`, and relied on "the loser gets `Err(InvalidTransition)`" to protect against a race — but that assumption was wrong for a real completion landing in the gap between the recheck and the second lock acquisition, since `Completed -> Completed` never errors. A real `Success`/`Failure` landing in that gap would have been silently overwritten by the synthetic `Stale` conclusion, with no self-heal (the real webhook already fired and won't fire again). Folding the recheck and the write into one lock acquisition closes this instead of narrowing it.
