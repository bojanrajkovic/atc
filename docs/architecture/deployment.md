...
- **`ATC_STALENESS_THRESHOLD`** — humantime duration (`48h`, `3d`), or unset the config-file key entirely / mount a file with the literal YAML `null` to disable. After this long with no observed webhook activity, a non-terminal run or job is force-completed with conclusion `Stale` through the normal outbox/NOTIFY write path in both storage modes, healing runs/jobs GitHub never sent a terminal webhook for. Default `48h`; hard floor of `6h` (below GitHub's own hosted-job ceiling — startup exits below the floor). Restart-only, same reload posture as the two knobs above. The chart always sets this env var (default `"48h"`) — disabling the sweep requires a config file with `staleness_threshold: null`, since figment's env layer takes precedence over the file and a plain string value can't express "unset". Set via `config.stalenessThreshold`. See [ADR-0013](../architecture-decisions/0013-staleness-sweep-synthetic-completion.md) for the design rationale.
