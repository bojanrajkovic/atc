...
- **`auth.mode` / `auth.github.*`** — configures the first-party GitHub OAuth login and session substrate (`ATC_AUTH__MODE`, `ATC_AUTH__GITHUB__CLIENT_ID`, `ATC_AUTH__GITHUB__CLIENT_SECRET`, `ATC_AUTH__GITHUB__PUBLIC_ORIGIN`, `ATC_AUTH__GITHUB__REPO_AUTH_TTL`, `ATC_AUTH__GITHUB__MAX_SESSION_TTL`). `auth.mode: github` requires an external PostgreSQL connection (sessions are Postgres-only) plus `existingSecret.authGithubClientSecret.name` + `.secretKey` for the OAuth client secret — no inline value is offered, unlike the webhook secret above. Render-time `{{ fail }}` guards enforce all four preconditions (Postgres, `auth.github.clientId`, `publicOrigin`, and the client-secret Secret reference); the `publicOrigin` guard additionally mirrors the server's own `validate_public_origin` shape checks (absolute `http`/`https`, no userinfo, no path beyond `""`/`"/"`, no query or fragment) via Sprig's `urlParse`, so a malformed origin fails at chart-render time instead of a pod crash-loop. `repoAuthTtl`/`maxSessionTtl` are pinned to the server's own defaults (`1h`/`30d`) and rendered unconditionally; `values.schema.json` rejects an empty or malformed override via the same humantime pattern used by the chart's other duration fields. This wires login/session **and** request-time enforcement of `GET /v1/state` and `GET /v1/ws` (session check + per-repository filtering, both read rails) — see [ADR-0014](../architecture-decisions/0014-native-github-auth-mode.md) and § Authentication below.
...
- **`ATC_STALENESS_THRESHOLD`** — humantime duration (`48h`, `3d`), or mount a file with the literal YAML `null` to disable. After this long with no observed webhook activity, a non-terminal run or job is force-completed with conclusion `Stale` through the normal outbox/NOTIFY write path in both storage modes, healing runs/jobs GitHub never sent a terminal webhook for. Default `48h`; hard floor of `24h` — GitHub's hosted queued-job wait ceiling, the longer of GitHub's two relevant hosted ceilings (6h running, 24h queued), since the sweep applies one threshold to both; startup exits below the floor. Restart-only, same reload posture as the two knobs above. Since figment's env layer always takes precedence over the config file, a plain env string can never express "disabled" — so the chart's `config.stalenessThreshold: null` instead renders `staleness_threshold: null` into the runner-pool-config ConfigMap (mounted even when `runnerPools` is empty) and omits the env var, letting the file layer's explicit `null` take effect. Set via `config.stalenessThreshold`. See [ADR-0013](../architecture-decisions/0013-staleness-sweep-synthetic-completion.md) for the design rationale.
...
ATC ships two independent auth postures (composing both in one deployment is out of scope): `auth.mode: none` (default) ships no built-in authentication at all — the SPA, `GET /v1/state`, and `GET /v1/ws` are unauthenticated, and the operator supplies a trusted network surface or a reverse proxy in front of the dashboard. `auth.mode: github` (ADR-0014) is a first-party GitHub OAuth login: users sign in with GitHub, ATC derives which repositories they can see directly from GitHub, and both read rails filter to that set — session check plus per-repository filtering on `GET /v1/state` and `GET /v1/ws`, exposed via the chart values documented above.

Setup guide, per-proxy recipes for `auth.mode: none`, the `auth.mode: github` GitHub App registration + coverage rule, cross-cutting gotchas (WebSocket `Origin` validation, cookie `SameSite`, idle-timeout starvation), and the path-split layout that lets `/v1/webhooks/github` bypass either mode: [`docs/operator/authentication.md`](../operator/authentication.md).
