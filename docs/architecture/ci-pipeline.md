...
**Rationale:** The verify-types step runs `cargo test --workspace -- export_bindings`, a non-instrumented build of the workspace. The coverage step runs `cargo llvm-cov --workspace`, a coverage-instrumented build. The two builds use different `RUSTFLAGS`, so cargo cannot reuse artifacts between them — each produces its own ~3-9 GB of `target/` content. With coverage running first, the second (verify-types) build has to add its artifacts to a target/ already swollen by llvm-cov instrumentation; cumulative footprint exceeded the GitHub-hosted runner's ~14 GB free disk and the linker failed with "No space left on device". Running verify-types first means the coverage build's instrumented artifacts are layered on top of a smaller baseline target/ — a single peak rather than two stacked. Tracking #55 for a deeper investigation (separate jobs, sccache, drop unused pre-installed software).
