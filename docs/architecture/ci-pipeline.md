...
Mend Renovate manages every dependency surface (Cargo, npm/pnpm, Dockerfile, docker-compose, GitHub Actions, mise) under one configuration at `renovate.json`. It extends `config:best-practices`, which bundles digest pinning for Docker FROMs and GitHub Action `uses:` references, dev-dependency pinning, weekly lockfile maintenance, the npm security minimum-release-age, and abandoned-package surfacing, plus `customManagers:dockerfileVersions`, which tracks the `Dockerfile`'s `ARG RUST_VERSION` / `ARG NODE_VERSION` defaults via `# renovate:` comments — the built-in `dockerfile` manager cannot see a partially interpolated tag like `rust:${RUST_VERSION}-slim`. Cross-manager `rust-toolchain` and `node-version` groups pair each ARG with its `.mise.toml` pin so both update in one PR. The mise manager resolves tools like `node` against the same docker datasource the Dockerfile uses for digest lookups, but `rangeStrategy` is overridden to `replace` for `mise` + `docker` matches: the global default of `pin` has no digest field to write into plain `tool = "version"` TOML, so a pin attempt finds the file unchanged and silently aborts the branch update — no PR, no error surfaced anywhere but the Renovate job log. One package is excluded from the `helm-values` manager: our own first-party image `ghcr.io/bojanrajkovic/atc` in `deploy/helm/atc/values.yaml`. The chart leaves `image.tag` empty so the deployment template resolves it to `.Chart.AppVersion`; digest-pinning an empty tag renders a malformed `repo:@sha256:...` reference (kubelet `InvalidImageName`), and pinning an image we build and release ourselves is redundant when `appVersion` already tracks the release.

The auto-merge policy: a 3-day release-age delay gives upstream time to yank or patch before ATC's CI sees the bump; security advisories (via `osvVulnerabilityAlerts: true`) bypass the delay and auto-merge non-major updates; major versions open a PR but require manual review. Several packages carry explicit no-automerge overrides — `ts-rs` and `sqlx` (bumps cross compile-time contracts: regenerated TypeScript types and sqlx macros), and the `opentelemetry ecosystem` group — every otel-coupled crate across four upstream repos (the opentelemetry-rust monorepo, `tracing-opentelemetry`, `opentelemetry-system-metrics`, `axum-otel-metrics`), bundled into one coordinated PR because 0.x bumps are breaking and must move in lockstep. The group matches on crate name rather than source URL: the opentelemetry-rust crates publish per-subdirectory `repository` deep-links that an exact `matchSourceUrls` against the bare repo root cannot catch. Conventional-commit prefix mapping: runtime dependency bumps land as `fix(deps):` (release-please ships a patch release); dev/tooling bumps land as `chore(deps):` (no release). See `renovate.json` for the full configuration.
