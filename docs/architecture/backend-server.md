...
- **`persist.rs`** (module at `src/persist/`) — In-tree home of `PgStore` until the `atc-store-pg` crate extraction lands. `InMemoryStore` now lives in the `atc-store-mem` crate (ADR 0008). The `PersistentStore` trait itself, `LivenessError`, the `PersistError` re-export, and the shared `join_with_timeout` helper live in the `atc-persist` crate (ADR 0008). `pub struct PgStore` + `impl PersistentStore for PgStore` — owns its connection pool, broadcast sender, `broadcast_watermark` / `last_drain_pass_at` atomics, and the listener + drain `JoinHandle`s (ADR 0006). Constructed via `PgStore::start(pool, listener_conn, shutdown)` / `PgStore::start_with_test_hooks(...)`; `pub(crate)` transaction helpers `upsert_run_in_txn`, `upsert_job_in_txn`, `insert_outbox_run_in_txn`, `insert_outbox_job_in_txn`, `notify_outbox_seq_in_txn`; `pub(crate)` read helpers `read_all_runs`, `read_all_jobs` used by the PG state handler (ADR 0005)

## Files

- `backend/crates/atc-server/src/main.rs` — Server entry point, config loading, tracing branching, router composition. Constructs `Arc<dyn PersistentStore>` via `PgStore::start` / `InMemoryStore::start` (ADR 0006); the per-mode background-task lifecycle is owned by the store, not main.
...
- `backend/crates/atc-server/src/persist/` — In-tree home of the `PgStore` impl until the `atc-store-pg` crate extraction lands. `InMemoryStore` now lives in `backend/crates/atc-store-mem/src/lib.rs` (test-support inspection helpers in `backend/crates/atc-store-mem/src/invariants.rs`); its private `spawn_eviction` associated function is called inside `InMemoryStore::start()`. The `PersistentStore` trait itself (ADR 0005, extended in ADR 0006 with `subscribe` + `shutdown`) lives in `backend/crates/atc-persist/src/lib.rs` (ADR 0008). `PgStore::start` spawns the per-store background tasks and stores the `JoinHandle`s on the returned `Arc<Self>`; `pub(crate)` transaction helpers for UPSERT+outbox+NOTIFY pattern; `pub(crate)` read helpers for PG state handler.
