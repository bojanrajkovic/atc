...
`AuthContext` (`auth.rs`) is the `FromRequestParts` extractor every auth-gated handler takes as an argument — `/v1/auth/me` today; `/v1/state` and `/v1/ws` adopt it for read-rail filtering (repo-set enforcement). It resolves to one of two variants:
...
- **`Session(SessionIdentity)`** — a valid session, carrying `github_login`, the authorized `repo_ids` (`HashSet<RepoId>` — the `atc-core` newtype from #449, not the design doc's simplified `HashSet<i64>` sketch, since callers compare it directly against `WorkflowRun::repo_id`), and `repos_refreshed_at` (needed for the staleness check below; threading it through the extractor avoids a second session lookup in callers that need it).

Extraction itself only ever produces `auth_required` — missing cookie, unknown cookie, or an expired session row all collapse to the same `{"reason": "auth_required"}` `401` via `AuthRejection::Required`, reusing the existing `session_from_cookie` helper (`get_cookie` + `SessionStore::load_session`). **Staleness is deliberately not checked during extraction** — `AuthContext::Session` is produced regardless of how old `repos_refreshed_at` is. `/v1/auth/me` needs to see a stale session to report it in its body (above); the read rails instead call `SessionIdentity::is_stale(repo_auth_ttl, now)` themselves and reject with `AuthRejection::Stale` (`{"reason": "stale_authorization"}`) when it's `true`. Both rejection variants — and their tracing (`reason`, `had_cookie`, never the cookie value) — live in one `IntoResponse` impl on `AuthRejection`, so the two reason strings (the wire contract `/v1/state`'s 401-aware connection handling keys off) are defined exactly once regardless of which handler triggers them.
