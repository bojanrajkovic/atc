...
`GET /v1/auth/me` reads the session cookie via `session_from_cookie` — a small helper kept as a plain function (not an extractor) rather than a request-time enforcement primitive so a later `AuthContext` extractor can wrap it instead of duplicating the cookie→session lookup. No session (missing or unknown/expired) is `401` with body `{"reason": "auth_required"}` — this exact string is a contract downstream consumers key off, not incidental phrasing. A valid session is `200` with `{"login", "repoCount", "reposRefreshedAt", "stale"}`; `stale` is `elapsed >= repo_auth_ttl` where `elapsed = now - repos_refreshed_at` — computed as a `DateTime - DateTime` subtraction and a `Duration` comparison, not `repos_refreshed_at + repo_auth_ttl >= now`. An unrepresentably large configured `repo_auth_ttl` falls back to `chrono::Duration::MAX` for "never stale"; adding that fallback to a `DateTime` would overflow-panic (`chrono`'s `Add` impl panics rather than saturates), which the elapsed-vs-ttl shape avoids entirely since it never adds a `Duration` to a `DateTime`.
