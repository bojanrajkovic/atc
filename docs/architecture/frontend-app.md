...
- **RunnerStore → TopBar → RunnerBar → RunnerPool → CapacityBar** — TopBar reads `RunnerStore` to compute pool statistics, passes pools[] array to RunnerBar, which renders RunnerPool items. Each RunnerPool branches off `pool.total.kind` in a runtime-exhaustive `switch` (one `default: const _: never = … throw` arm) into three rendering paths: `Bounded` renders CapacityBar + `running/N` count; `Unbounded` renders the running count plus an unbounded affordance (icon + accessible label, distinguishable from `Undeclared` via content per WCAG SC 1.4.1); `Undeclared` renders the running count only. RunnerPool receives `isActiveFilter` prop from RunnerBar (derived from `uiStore.activePoolFilter`) and shows accent border when true.
...
**RunnerPoolDisplay**
- `label: string` — Pool display name. TopBar's `displayGroupName(pool)` helper maps `groupName` to `null` when it's either literally `null` or the placeholder string `'Default'` (GitHub uses `"Default"` as the runner-group name for any job not placed in a custom group — it carries no operator-meaningful information; see issue #143). The label decision tree then becomes: `displayGroupName(pool) ?? labels.join(', ')` for unique cases, with `${groupName} · ${labels.join(', ')}` disambiguation when ≥2 pools share the same display name. The same `displayGroupName` feeds the count loop so the set of names we count matches the set we'd actually use as a display label.
- `running: number` — Count of running jobs in pool
- `queued: number` — Count of queued jobs in pool
- `total: RunnerPoolTotal` — Three-state declared capacity, populated by `computePoolStats`'s merge against `runStore.runnerPoolCapacities`. Adjacent-tagged union: `{ kind: 'Bounded'; value: number }` for pools whose operator declared an integer capacity (renders with a `CapacityBar`), `{ kind: 'Unbounded' }` for pools the operator declared with `capacity: null` (renders with an icon + accessible-label affordance, no bar), `{ kind: 'Undeclared' }` for pools observed via webhooks but not declared (count only). The runtime `switch` in `RunnerPool.svelte` includes a `default: const _: never = … throw` arm so an off-shape value from a boundary surfaces immediately instead of silently rendering `undefined` (see frontend-CLAUDE.md sharp-edge guidance for typed-union switches).
...
- Derives: three sorted arrays (`queuedRuns` ascending by createdAt, `inProgressRuns` descending by runStartedAt, `completedRuns` descending by updatedAt), each with run.id tie-breaker; direct lexical ISO-8601 comparison, no Date parsing. `completedRuns` additionally applies the display-TTL filter — see `displayTtlSeconds` below.
...
- **`jobs: $derived.by<Job[]>`** — flat view across all runs. Iterates `this.jobsByRun.values()` and concatenates each `Job[]` into a single array, applying the same display-TTL predicate as `completedRuns` so aged-out completed jobs do not appear in the flat view or in `runnerStore.pools`. The per-run `jobsByRun` map stays unfiltered — `RunDetailPanel` needs the full job list for any visible run, and run-level filtering already culls the runs whose jobs would otherwise show.
- **`runnerPoolCapacities: RunnerPoolCapacity[]`** (`$state`) — Operator-declared pool ceilings from the latest snapshot. Replaced atomically by `loadSnapshot(runs, jobs, runnerPoolCapacities = [])`; the third argument defaults to `[]` so older snapshots during a rolling deploy (or callers that don't pass it) do not break the load. Cleared by `clear()`.
- **`displayTtlSeconds: number`** (`$state`) — Operator-configured display TTL stamped onto the snapshot by `state_handler` (default `1h` via `ATC_DISPLAY_TTL`). Replaced atomically by `loadSnapshot(runs, jobs, capacities, displayTtlSeconds = 0)`; the fourth argument defaults to `0` so a snapshot from a pre-feature replica during a rolling deploy keeps every completed row visible. The `completedRuns` and flat `jobs` derivers compose this against `uiStore.nowMs` and per-row `completedAt`, so completed rows age out reactively without an event arriving. The filter predicate keeps the row visible when `displayTtlSeconds === 0`, when status is not `Completed`, or when `completedAt` is missing/null (mixed-version snapshot tolerance); only when all three escape hatches miss does it compare `now - completedAt > ttl`. The strict `>` mirrors the server SQL's `completed_at >= cutoff` exactly — predicate-parity unit tests on both sides verify agreement on the boundary tuple `completed_at + ttl == now`. **Clock-skew model:** server and client cutoffs are computed from independent wall clocks. The inconsistency band at the boundary equals NTP skew, which is sub-second and dwarfed by typical TTL values; no alignment effort is made. See [ADR 0009](../architecture-decisions/0009-display-vs-data-retention.md).
...
- `computePoolStats(jobs: Job[], capacities: readonly RunnerPoolCapacity[] = []): RunnerPoolStats[]` is exported from the same module as a pure function. It dedupes labels via `new Set(...)`, sorts them, uses `JSON.stringify(sortedLabels)` as a collision-free map key, skips jobs with status `Waiting` or `Completed`, increments `queued` / `running` per status, and sets `groupName` from the most recent observed `runner.groupName`. On first sight of each pool, it merges `capacities` into `RunnerPoolStats.total` as a `RunnerPoolTotal` adjacent-tagged enum: `{ kind: 'Bounded', value: n }` when the operator declared an integer capacity, `{ kind: 'Unbounded' }` when the operator declared `capacity: null`, `{ kind: 'Undeclared' }` when the pool has no matching declaration. The capacity lookup keys via `poolKey()` (ADR 0001) so wire-side sort order doesn't have to match JS sort order — both sides funnel through the frontend canonicalization before keying. Runner group identity does not participate in `total` classification; bounded/unbounded is the operator-declared rail and undeclared is the webhook-observed rail. Returns the resulting array sorted lexicographically by labels.
- The derivation chain is: `runStore.jobsByRun` + `runStore.runs` + `runStore.runnerPoolCapacities` → `runnerStore.pools`. Pool state self-heals on every run, job, or snapshot reload event without any explicit dispatch into the runner store.
- See ADR 0004 (`docs/architecture-decisions/0004-frontend-derived-pool-stats.md`) for the rationale (multi-replica concurrent-writer correctness). Operator-declared capacity is composed onto the snapshot at the route layer (`AppState`) — not derived in either store — and merged here.
...
`uiStore.lastTriggerRunId` (the focus-restoration trigger consumed by `RunDetailPanel.onCloseAutoFocus`) is touched only in the run-to-run branch, where it's updated to match the new `selectedRunId` so close-focus restoration lands on the displayed card and not a stale prior trigger. The close-the-panel branches (`parsed === null` and stale) leave it alone — the originally-clicked card is still the right focus target when the panel closes.
...
- `this.jsonReviver` includes `'lastSeq'` in the bigint conversion allowlist alongside `'seq'`, `'id'`, `'runId'`, `'jobId'`, and `'number'`
...
- `frontend/e2e/lib/ws-mock.ts` — Shared Playwright harness: `WS_MOCK_INIT_SCRIPT`, `makeRunEvent`, `makeJobCommittedEvent`, `sendWS`, `sendWSBatch`, `sendWSBatchPaced`, `randomBatchSchedule` — intercepts `new WebSocket('/v1/ws')` and routes events through `window.eventDispatcher`. The harness exposes three send paths: `sendWS()` calls `dispatch` then `flush` synchronously (deterministic, used when a test needs exactly one event delivered before asserting); `sendWSBatch()` calls `dispatch` for each event without flushing between them and lets natural RAF coalesce the batch, then awaits a `bufferLength === 0` synchronization fence followed by one extra RAF tick (used by aria-live burst scenarios with ≤ 4 events where pacing is irrelevant); `sendWSBatchPaced()` drains events across asynchronous boundaries one slice per tick (driver: `'raf'` default, `'macrotask'`, or `{ intervalMs }`) so the dispatcher's flush rAF runs between pacing ticks, exercising coalescing under realistic randomized per-tick arrival pacing (used by frame-budget). `randomBatchSchedule(total, { min, max, seed })` pre-computes a seeded LCG batch schedule for reproducible CI traces.
...
A Playwright test that fires 1000 events through the live `EventDispatcher` (via `sendWSBatchPaced` with a seeded randomized schedule) while a Chrome DevTools Protocol trace is running. Trace data is collected via a CDP session (`context.newCDPSession(page)`) with the `devtools.timeline,rendering` categories. After the burst, the test parses `AnimationFrame` event timestamps, computes a summary (`p50_ms`, `p95_ms`, `p99_ms`, `dropped_frames`), and saves the result — alongside the seeded schedule itself — to `frontend/test-results/frame-budget-trace.json`.

**What this measures.** Tier 2 measures dispatcher rAF coalescing under randomized per-tick injection across many rAF ticks. `sendWSBatchPaced` splits the 1000-event burst into ~12–30 variable-size slices (`min=10, max=100`, seeded LCG so the schedule is reproducible) and paces them on `requestAnimationFrame`. Inside each pacing tick, the first `dispatch()` schedules the dispatcher's flush rAF; that flush rAF is queued before the next pacing rAF (FIFO order at the next frame), so the cadence is "drain previous-slice → inject next-slice" per frame — exactly what real WS arrival pacing looks like. The deltas therefore reflect dispatcher coalescing, not a synchronous injection-loop tax. Slices near the upper bound (100 events) may exceed one frame budget — that's the realistic WS-burst case captured as `dropped_frames`. Tier 2 is orthogonal to Tier 1: Tier 1 (`dispatcher.perf.browser.test.ts`) owns the deterministic rAF-coalescing gate via a manually-driven rAF queue (`flushSpy.mock.calls.length === 10` for 1000 events in 10 batches); Tier 2 surfaces real-browser pacing behavior under a randomized schedule.

**What dominates `dropped_frames` (and what the metric is good for).** The dispatcher's own hot path (`dispatch` → `processBuffer` → `routeEvent` → `applyRunEvent`) is O(N) with a small constant; per-event work is microseconds. The wall-clock cost of draining a large slice is dominated by **non-virtualized Kanban column rendering**: `KanbanColumn.svelte` renders every run as a live DOM node, and `animate:flip` runs a per-sibling FLIP measurement pre-pass on each list mutation — so a 99-event slice inserted into a column already holding hundreds of cards triggers an O(N) layout read across all siblings, not just the new ones. List windowing is tracked in [issue #37](https://github.com/bojanrajkovic/atc/issues/37); this trace artifact provides the synthetic E2E stress measurement called out in that issue's acceptance criteria. Until windowing lands, `dropped_frames` should be read as a baseline of the Kanban render path under burst load — useful for catching a regression in `applyRunEvent` cost or RunCard mount cost, but not a budget the dispatcher alone can move.
