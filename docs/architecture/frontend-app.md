...
`LoginScreen` auto-attempts a silent re-auth for *either* auth reason the moment `connectionStore.authReason` becomes non-null — not just `stale_authorization`. A bare page load never has transient user activation to spend, but this cannot be assumed reliably blocked as a result: empirically (verified via the Playwright e2e suite), `window.open` can still succeed on a cold `auth_required` visit in some browser/automation contexts, not just when an already-active SPA session gets revoked out from under a user mid-use. The two reasons diverge only in what happens when the popup is blocked (see below) — never in whether it's attempted.

```mermaid
flowchart LR
    S["authReason becomes\nnon-null"] --> O{"window.open(popup)\nsucceeds?"}
    ...
    O -- "no (no gesture)" --> F{"reason ="}
    F -- stale_authorization --> F1["location.href = login\nwith return_to"]
    F -- auth_required --> F2["stay put — button\nis the only path"]
```

`window.open` is called synchronously inside the `$effect` that observes the reason change (wrapped in a `try`/`catch` — a sandboxed embedding without `allow-popups` can throw rather than return `null`) — calling it from an async continuation would already have lost any transient user activation, so there would be nothing to check. When a popup does open, the callback page (server-side, `POPUP_CALLBACK_HTML` in `auth.rs`) posts `'session-refreshed'` on a `BroadcastChannel` named `'atc-auth'` and self-closes; `LoginScreen` never sends anything back, it only listens and then calls `connectionStore.retry()`, which is what actually re-fetches the snapshot and reopens the WS — the popup round-trip never touches the main tab's connection.

**When `window.open` is blocked (returns `null` or throws), the two reasons diverge:** `stale_authorization` auto-redirects the whole tab (`location.href = login`) — the tab already has a working session and this is often an unattended reconnect with nobody watching, so self-healing without a click is the point. `auth_required` does *not* auto-redirect — a fresh visitor who's never seen this app's GitHub consent screen getting silently navigated into it, with no warning and no way back (the classic redirect-loop trap: cancel or hit back, land right back on the login screen, get auto-redirected again with no resting state ever reachable) is worse than requiring a click. The button is the only path there, same as before this feature existed.
...
`popupInFlight` (a `$state` field, read `untrack`'d inside the effect's own guard check to avoid a read-your-own-write dependency cycle) guards against opening a second popup while one is already open. It does **not** gate the manual "Sign in with GitHub" link — the link is always clickable. Since the backend's OAuth flow cookie is a single slot per browser, not scoped per popup/tab, a concurrent popup-and-manual-click pair would otherwise overwrite each other's flow and fail one with a state mismatch; rather than disable the button until the popup resolves (which traps the user behind a dead control if the popup silently never completes — the exact failure an early version of this feature hit in e2e testing, since Playwright's automation grants `window.open` more readily than a human browsing session would), the manual click cancels (`cancelPopup?.()`) whatever popup is currently in flight — closing its window and clearing its channel/timers — before proceeding with its own redirect. The click always wins deterministically; there is never a state where the only visible control is unusable.
