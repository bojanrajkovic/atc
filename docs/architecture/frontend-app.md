...
- **`jobs: $derived.by<Job[]>`** — flat view across all runs. Iterates `this.jobsByRun.values()` and concatenates each `Job[]` into a single array. Consumed by `runnerStore.pools` (`computePoolStats(runStore.jobs, runStore.runnerPoolCapacities)`) so the runner-pool derivation has a stable single dependency rather than threading the per-run `SvelteMap` itself.
