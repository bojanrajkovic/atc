...
- `total: number | null` — Operator-declared capacity, populated by `computePoolStats`'s merge against `runStore.runnerPoolCapacities`. `null` for any pool the operator hasn't declared — those render without a `CapacityBar`.
- `isElastic: boolean` — Whether pool auto-scales (derived from runner `group_id == Some(0)`). Currently has no display consumer; the wire field is preserved for any future non-display consumer. The chip-label rule above is intentionally a pure function of `(groupName, labels)` only — operator-declared elasticity is deferred (see follow-up).
...
- **`StateSnapshot.accessibleReposCount: bigint`** — Reports the size of the caller's accessible repository set when auth scoping is active on the server. The field is `#[serde(default)]` on the Rust side (deserializes to `0n` from snapshots that lack it, for rolling-deploy tolerance) and is composed at the handler layer, not the persistent store. The frontend does not yet consume this value; test fixtures that construct `StateSnapshot` literals must initialize it to `0n` to satisfy the generated type.
...
- `computePoolStats(jobs: Job[], capacities: readonly RunnerPoolCapacity[] = []): RunnerPoolStats[]` is exported from the same module as a pure function. It dedupes labels via `new Set(...)`, sorts them, uses `JSON.stringify(sortedLabels)` as a collision-free map key, skips jobs with status `Waiting` or `Completed`, increments `queued` / `running` per status, sets `groupName` from the most recent observed `runner.groupName`, sets `isElastic = true` whenever any observed `runner.groupId === 0n` (bigint-aware), and merges `capacities` into `total` on first sight of each pool. The capacity lookup keys via `poolKey()` (ADR 0001) so wire-side sort order doesn't have to match JS sort order — both sides funnel through the frontend canonicalization before keying. Pools without a declaration stay `total: null`. Returns the resulting array sorted lexicographically by labels.
