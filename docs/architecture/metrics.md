...
**Log/trace correlation.** `trace_id`/`span_id` (hex, matching OTel wire format) are recorded onto the blanket `http.request` span (see [Span inventory](#span-inventory)) once its OTel span context is finalized. Because JSON logging has `with_span_list(true)` enabled, every log line nested under a request — which is effectively every request-handling log ATC emits — carries these fields, letting an operator jump from a log line to the matching trace in Tempo/Honeycomb (e.g. via a Grafana Derived Field on `trace_id`). The fields are declared as `tracing::field::Empty` and stay absent from output when OTel isn't configured, since `Span::context()` then yields an invalid span context. This does not extend to spans rooted outside a request (e.g. `outbox.heartbeat.tick`, `drain.pass`) — those still export their own independent OTel trace, but nothing stamps it onto their log output.
