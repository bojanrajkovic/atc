...
2. **Auth app — new, metadata-only.** Register a GitHub App for login:
   ...
   - **Expire user authorization tokens:** leave **on** (the default for new Apps) — ATC's OAuth exchange assumes a short-lived access token; it's discarded within the same request regardless, but a shorter GitHub-side lifetime is defense in depth.
