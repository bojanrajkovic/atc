...
Last verified: 2026-05-06 (Phase 5 operational metrics chunk: six metrics on the Postgres drain path — outbox lag, drain-pass duration, wake-coalesce, drain startup, watermark, min_pending_seq — plus per-metric documentation, metric authoring contract, and Grafana template dashboard at `deploy/grafana/atc-postgres-overview.json`)
